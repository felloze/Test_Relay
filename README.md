...
- **触点类型识别**：长按功能键，自动完成「释放采样常闭 → 吸合采样常开」流程，判定 8 种触点组合（1A/1B/1C/2A/2B/2C/AB/BA），TM1650 数码管显示识别结果（1s 后恢复电压显示）
- **压力测试**：识别完成后自动开启。吸合/释放两种状态下持续监测触点，任一判据不满足即驱动蜂鸣器报警，用于老化/寿命测试
- **测试键**：点动模式（跟随按下/释放）与自锁模式（按一下翻转）两种，由功能键双击切换
